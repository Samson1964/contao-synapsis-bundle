...
class ForumListener
{
    ...
    /**
     * Nur echte Mitgliedergruppen (ohne die fiktive Gaeste-Gruppe) - fuer das
     * Umfragen-Erstellrecht (Gaeste koennen keine Umfragen anlegen).
     *
     * @return array<int, string>
     */
    public function getMemberGroupOptions(): array
    {
        ...
        foreach ($this->connection->fetchAllAssociative('SELECT id, name FROM tl_member_group ORDER BY name') as $row) {
            $options[(int) $row['id']] = $row['name'];
        }
